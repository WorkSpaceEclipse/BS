...
public class TxtFragemnt extends BaseFragment {
	/** 标志位，标志已经初始化完成 */
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		if (view == null) {
			view = inflater.inflate(R.layout.txt_fragment, container, false);
	...
	protected void addData() {
		// 翻页
		JsonDownLoad.getJson(Const.TXT_URL + page, false, new JsonCallBack() {
			// 获取json
			@Override
			public void jsonBack(String jsonStr) {
				ArrayList<BDJObj> data = new ArrayList<BDJObj>();
				data = JsonTools.getBds(jsonStr);
				LogUtil.i("------>TxtFragment翻到第" + page + "页");
				isUpdata = false;
				// 更新LV在adapter里更新IMG
				if (MyApplication.currentFragment == Const.TXT_FRAGMENT_INDEX) {

					Message msg = new Message();
					msg.obj = data;
					msg.what = Const.TXT_FRAGMENT_UPDATEVIEW;
	...
	private void initView() {
		lv = (ListView) view.findViewById(R.id.listView_txtfragment);
		/*
	...
	private void initData() {
		JsonDownLoad.getJson(Const.TXT_URL + page, true, new JsonCallBack() {
			// 获取json
			@Override
			public void jsonBack(String jsonStr) {
				mHasLoadedOnce = true;
	...
	public static void updateView(ArrayList<BDJObj> obj) {
		...
		HomeFragment.setCurrentFragment(Const.TXT_FRAGMENT_INDEX);
		if (adapter == null) {
			adapter = new TxtLvAdapter(obj);
			lv.setAdapter(adapter);
		} else {
			adapter.upData(obj);
		}
